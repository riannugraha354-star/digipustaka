    ...
    public function wa($id)
{
    ...
    // 1. Ambil nomor telepon (Pastikan nomor diawali 62 atau gunakan pembersihan string)
    $no_hp = $user['telepon']; 

    // 2. Siapkan pesan otomatis (Gunakan rawurlencode agar spasi/karakter aman di URL)
    $pesan = "Halo " . $user['nama'] . ", ini dari Perpustakaan DigiPustaka. Ada informasi penting terkait akun/peminjaman Anda.";
    $pesan_encoded = rawurlencode($pesan);

    // 3. Buat link WhatsApp API
    // Kita gunakan api.whatsapp.com agar kompatibel di HP maupun Web
    $url_wa = "https://api.whatsapp.com/send?phone=" . $no_hp . "&text=" . $pesan_encoded;

    // 4. Redirect ke WhatsApp
    return redirect()->to($url_wa);
}
}
